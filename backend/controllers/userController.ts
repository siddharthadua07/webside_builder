...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        //   Enhance user prompt 
        const promptEhanceResponse = await openai.chat.completions.create({
...
`
        ...
        const enhancedPrompt = promptEhanceResponse.choices[0].message.content;
        await prisma.conversation.create({
            data: {
                role: 'assistant',
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: 'now generating your webside....',
                projectId: project.id
            }
        })
        //  generate webside code 
...
//  Controllers functions to Get all user Projects 
...
export const purchesCredits = async (req: Request, res: Response) => {
    try {
        interface Plan {
            credits: number;
            amount: number;
            ...
            line_items: [
                {
                    price_data: {
                        currency: 'usd',
                        product_data: {
                            name: `AiSiteBilder - ${plan.credits} Credits`,
                        },
